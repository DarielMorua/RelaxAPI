...
async function crearFavorito(req, res) {
    try {
        ...
        res.status(200).json({ message: "Profesional agregado como favorito y actualizado con éxito" });
    } catch (error) {
        res.status(400).json({ message: "Error al agregar profesional como favorito", error: error.message });
    }
};
...
async function buscarFavoritos(req, res) {
    try {
        ...
        const favoritos = await Favorite.find({ userId: userId }).populate('professionalId');

        // Si no hay favoritos
        if (favoritos.length === 0) {
            return res.status(404).json({ message: "No se encontraron profesionales favoritos para este usuario." });
        }

        // Responder con la información de los profesionales favoritos
        res.status(200).json(favoritos);
    } catch (error) {
        res.status(500).json({ message: "Error al obtener los profesionales favoritos", error: error.message });
    }
};
...
async function removerFavoritos(req, res) {
    try {
        ...
        if (!userId || !professionalId) {
            return res.status(400).json({ message: "userId y professionalId son requeridos" });
        }

        // Eliminar el favorito
        const deletedFavorite = await Favorite.findOneAndDelete({ userId: userId, professionalId: professionalId });

        // Si no se encontró el favorito
        if (!deletedFavorite) {
            return res.status(404).json({ message: "No se encontró este favorito para eliminar" });
        }
        ...
        res.status(200).json({ message: "Favorito eliminado y profesional actualizado con éxito" });
    } catch (error) {
